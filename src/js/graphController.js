...
export default class graphController {
    ...
    callTele() {
        this._$http({
            ...
            const min = data.readings[0][1];
            const max = data.readings[0][1];
            const start = data.readings[0][0];
            const end = data.readings[data.readings.length - 1][0];
            const j = 0;
            ...
            const meanTimes = [];
            const lastPoint = (data.readings[0][0]);
            ...
            const drag = d3.drag()
                .on('drag', (d, i) => {
                    const x0 = xScale.invert(d3.mouse(this)[0]).getTime();
